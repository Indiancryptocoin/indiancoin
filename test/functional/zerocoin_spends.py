...
class ZerocoinSpendTest(IndianCoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zind_bal, ind_bal):
            zind_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zind_bal)
            ind_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], ind_bal)
            return zind_bal, ind_bal
        ...
        zind_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zind_balance, 6666)
        ...
        zind_balance, balance = check_balances(denom_0, zind_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zind_balance, balance = check_balances(denom_2, zind_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zind_balance, balance = check_balances(denom_3, zind_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zind",
        ...
        _, _ = check_balances(denom_1, zind_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
