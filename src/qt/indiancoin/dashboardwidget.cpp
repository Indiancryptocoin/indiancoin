...
#include "qt/indiancoin/dashboardwidget.h"
#include "qt/indiancoin/forms/ui_dashboardwidget.h"
#include "qt/indiancoin/sendconfirmdialog.h"
#include "qt/indiancoin/txrow.h"
...
DashboardWidget::DashboardWidget(INDIANCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of IND and zIND staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareInd, "square-chart-ind");
    setCssProperty(ui->labelSquarezInd, "square-chart-zind");
    setCssProperty(ui->labelInd, "text-chart-ind");
    setCssProperty(ui->labelZind, "text-chart-zind");
    ...
    ui->labelAmountZind->setText("0 zIND");
    ui->labelAmountInd->setText("0 IND");
    setCssProperty(ui->labelAmountInd, "text-stake-ind-disable");
    setCssProperty(ui->labelAmountZind, "text-stake-zind-disable");
    ...
    ui->btnHowTo->setText(tr("How to get IND or zIND"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isInd = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZIND;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair IND, zIND
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ind = (pair.first != 0) ? pair.first / 100000000 : 0;
            zind = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalInd += pair.first;
            chartData->totalZind += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesInd.append(ind);
        chartData->valueszInd.append(zind);

        int max = std::max(ind, zind);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("IND");
    set1 = new QBarSet("zIND");
    ...
    set0->append(chartData->valuesInd);
    set1->append(chartData->valueszInd);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalInd > 0 || chartData->totalZind > 0) {
        setCssProperty(ui->labelAmountInd, "text-stake-ind");
        setCssProperty(ui->labelAmountZind, "text-stake-zind");
    } else {
        setCssProperty(ui->labelAmountInd, "text-stake-ind-disable");
        setCssProperty(ui->labelAmountZind, "text-stake-zind-disable");
    }
    forceUpdateStyle({ui->labelAmountInd, ui->labelAmountZind});
    ui->labelAmountInd->setText(GUIUtil::formatBalance(chartData->totalInd, nDisplayUnit));
    ui->labelAmountZind->setText(GUIUtil::formatBalance(chartData->totalZind, nDisplayUnit, true));
