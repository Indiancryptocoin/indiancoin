...
#include "qt/indiancoin/send.h"
#include "qt/indiancoin/forms/ui_send.h"
#include "qt/indiancoin/addnewcontactdialog.h"
#include "qt/indiancoin/qtutils.h"
#include "qt/indiancoin/sendchangeaddressdialog.h"
#include "qt/indiancoin/optionbutton.h"
#include "qt/indiancoin/sendconfirmdialog.h"
#include "qt/indiancoin/myaddressrow.h"
#include "qt/indiancoin/guitransactionsutils.h"
...
#include "zindcontroldialog.h"

SendWidget::SendWidget(INDIANCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("IND");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zIND");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (IND) or private coins (zIND)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("INDIANCOIN address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 IND");
    ...
    setCssProperty(coinIcon, "coin-icon-ind");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onINDSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onINDSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZind = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZind));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZind ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendInd = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendInd ? AskPassphraseDialog::Context::Send_IND : AskPassphraseDialog::Context::Send_zIND, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendInd) ? send(recipients) : sendZind(recipients)) {
...
bool SendWidget::sendZind(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zIND is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zIND selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZIndControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZIndControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZind(
        ...
        inform(tr("zIND transaction sent!"));
        ZIndControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZIND_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zIND require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zIND transaction
        ...
        emit message("zIND transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isIND){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any IND to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZIndControlDialog *zIndControl = new ZIndControlDialog(this);
            zIndControl->setModel(walletModel);
            zIndControl->exec();
            ui->btnCoinControl->setActive(!ZIndControlDialog::setSelectedMints.empty());
            zIndControl->deleteLater();
        } else {
            inform(tr("You don't have any zIND in your balance to select."));
        }
    }
}
...
void SendWidget::onINDSelected(bool _isIND){
    isIND = _isIND;
    setCssProperty(coinIcon, _isIND ? "coin-icon-ind" : "coin-icon-zind");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress indAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(indAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(indAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
