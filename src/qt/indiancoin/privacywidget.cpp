...
#include "qt/indiancoin/privacywidget.h"
#include "qt/indiancoin/forms/ui_privacywidget.h"
#include "qt/indiancoin/qtutils.h"
#include "guiutil.h"
#include "qt/indiancoin/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(INDIANCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zIND anonymizes your IND by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zIND or convert back to IND"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 IND ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zIND");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zIND");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zIND");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zIND");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zIND");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zIND");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zIND");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zIND");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zIND Movements"));
    ...
    // Only Convert to IND enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzIND->setTitleClassAndText("btn-title-grey", "Total 0 zIND");
    ui->btnTotalzIND->setSubTitleClassAndText("text-subtitle", "Show denominations of zIND owned.");
    ui->btnTotalzIND->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select IND outputs to mint into zIND.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zIND");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzIND, SIGNAL(clicked()), this, SLOT(onTotalZindClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zIND");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of IND to mint into zIND"));
    }else{
        btnText = tr("Convert back to IND");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zIND to convert back into IND"));
    }
...
void PrivacyWidget::onTotalZindClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzIND->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzIND->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zIND is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zIND, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zIND").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zIND minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZind(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zIND converted back to IND"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zIND </b>";
    ...
    ui->btnTotalzIND->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
