...
#include "qt/indiancoin/topbar.h"
#include "qt/indiancoin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/indiancoin/lockunlock.h"
#include "qt/indiancoin/qtutils.h"
#include "qt/indiancoin/receivedialog.h"
...
TopBar::TopBar(INDIANCOINGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopInd, ui->labelAmountTopzInd}, "amount-small-topbar");
    setCssProperty({ui->labelAmountInd, ui->labelAmountzInd}, "amount-topbar");
    setCssProperty({ui->labelPendingInd, ui->labelPendingzInd, ui->labelImmatureInd, ui->labelImmaturezInd}, "amount-small-topbar");
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("INDIANCOIN")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // IND Total
    CAmount indAvailableBalance = balance;
    // zIND Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalInd = GUIUtil::formatBalance(indAvailableBalance, nDisplayUnit);
    QString totalzInd = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopInd->setText(totalInd);
    ui->labelAmountTopzInd->setText(totalzInd);

    // Expanded
    ui->labelAmountInd->setText(totalInd);
    ui->labelAmountzInd->setText(totalzInd);

    ui->labelPendingInd->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzInd->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureInd->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezInd->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
