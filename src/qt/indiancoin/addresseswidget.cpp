...
#include "qt/indiancoin/forms/ui_addresseswidget.h"
#include "qt/indiancoin/addresslabelrow.h"
#include "qt/indiancoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(INDIANCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter INDIANCOIN address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress indAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(indAdd.Get(), label.toUtf8().constData(),
                indAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
