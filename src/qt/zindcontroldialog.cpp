...
#include "ui_zindcontroldialog.h"
...
std::set<std::string> ZIndControlDialog::setSelectedMints;
std::set<CMintMeta> ZIndControlDialog::setMints;

bool CZIndControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZIndControlDialog::COLUMN_DENOMINATION || column == ZIndControlDialog::COLUMN_VERSION || column == ZIndControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZIndControlDialog::ZIndControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZIndControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zIND Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZInd->setProperty("cssClass", "text-main-purple");
    ui->labelZInd_int->setProperty("cssClass", "text-main-purple");
...
ZIndControlDialog::~ZIndControlDialog()
{
    delete ui;
}

void ZIndControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZIndControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZIndControlWidgetItem* itemDenom(new CZIndControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZIndControlWidgetItem *itemMint = new CZIndControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zIND.";
            else if (!mint.isSeedCorrect)
                strReason = "The zIND seed used to mint this zIND is not the same as currently hold in the wallet";
            else
...
void ZIndControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZIndControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZInd_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZIndControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZIndControlDialog::GetSelectedMints()
{
...
void ZIndControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
