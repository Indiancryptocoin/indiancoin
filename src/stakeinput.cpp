...
CZIndStake::CZIndStake(const libzerocoin::CoinSpend& spend)
{
...
int CZIndStake::GetChecksumHeightFromMint()
{
...
int CZIndStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZIndStake::GetChecksum()
{
    return nChecksum;
}

// The zIND block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZIndStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZIndStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zIND staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZIndStake::GetUniqueness()
{
    //The unique identifier for a zIND is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZIndStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZIndStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zIND that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZINDOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zIND output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zIND", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZINDOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zIND output", __func__);
...
bool CZIndStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZIndStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzINDTracker* zindTracker = pwallet->zindTracker.get();
    CMintMeta meta;
    if (!zindTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zindTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!IND Stake
bool CIndStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CIndStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CIndStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CIndStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CIndStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CIndStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CIndStake::GetUniqueness()
{
    //The unique identifier for a IND stake is the outpoint
...
CBlockIndex* CIndStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
