...
public:
    ...
    bool WriteZINDSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZINDSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZINDSeed_deprecated(uint256& seed);
    bool EraseZINDSeed();
    bool EraseZINDSeed_deprecated();

    bool WriteZINDCount(const uint32_t& nCount);
    bool ReadZINDCount(uint32_t& nCount);
