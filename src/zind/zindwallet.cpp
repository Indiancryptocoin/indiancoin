...
CzINDWallet::CzINDWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zind seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZINDSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZINDSeed_deprecated()) {
                    LogPrintf("%s: Updated zIND seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zind seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zind wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzINDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZINDCount(nCountLastUsed);
    else if (!walletdb.ReadZINDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzINDWallet::Lock()
{
    seedMaster = 0;
}

void CzINDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzINDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZIND(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzINDWallet::LoadMintPoolFromDB()
{
...
void CzINDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzINDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzINDWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zindTracker->HasPubcoinHash(pMint.first)) {
...
bool CzINDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZIND(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zindTracker which also adds to database
    pwalletMain->zindTracker->Add(dMint, true);
...
void CzINDWallet::SeedToZIND(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzINDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzINDWallet::GenerateDeterministicZIND(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzINDWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzINDWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzINDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
