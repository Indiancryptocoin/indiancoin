// Copyright (c) 2018-2020 The INDIANCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zind/deterministicmint.h>
#include "zindtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzINDTracker::CzINDTracker(std::string strWalletFile)
{
...
CzINDTracker::~CzINDTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzINDTracker::Init()
{
...
bool CzINDTracker::Archive(CMintMeta& meta)
{
...
bool CzINDTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzINDTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzINDTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzINDTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzINDTracker::GetSerialHashes()
{
...
CAmount CzINDTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzINDTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzINDTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzINDTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzINDTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzINDTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzINDTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzINDTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzINDTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzINDTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzINDTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzINDWallet* zINDWallet)
{
    bool iszINDWalletInitialized = (NULL != zINDWallet);
    CMintMeta meta;
    ...
    if (! iszINDWalletInitialized)
        zINDWallet = new CzINDWallet(strWalletFile);
    meta.isSeedCorrect = zINDWallet->CheckSeed(dMint);
    if (! iszINDWalletInitialized)
        delete zINDWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzINDTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzINDTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzINDTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzINDTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzINDTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzINDTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzINDWallet* zINDWallet = new CzINDWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zINDWallet);
        }
        delete zINDWallet;
        LogPrint("zero", "%s: added %d dzind from DB\n", __func__, listDeterministicDB.size());
    }
