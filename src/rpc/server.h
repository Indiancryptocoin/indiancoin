...
/**
 * INDIANCOIN RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZindSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzindseed(const UniValue& params, bool fHelp);
extern UniValue getzindseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzind(const UniValue& params, bool fHelp);
extern UniValue dzindstate(const UniValue& params, bool fHelp);
