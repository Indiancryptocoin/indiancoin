...
*Setup instructions for a gitian build of INDIANCOIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the INDIANCOIN
...
to the INDIANCOIN GitHub release page.
...
- [Building INDIANCOIN Core](#building-indiancoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/indiancoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/indiancoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/indiancoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/indiancoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/indiancoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/indiancoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/indiancoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@indiancoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/indiancoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/indiancoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/indiancoin-osx-*-build.assert
```
